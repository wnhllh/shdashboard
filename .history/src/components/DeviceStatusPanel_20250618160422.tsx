...
const WafStatusComponent: React.FC = () => {
  ...
  return (
    <div className="relative bg-gradient-to-br from-teal-900/30 via-slate-900/80 to-emerald-900/30 backdrop-blur-sm p-2 rounded-lg border border-teal-400/40 shadow-[0_0_12px_rgba(20,184,166,0.12)] overflow-hidden">
      {/* 背景装饰 */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_80%,rgba(20,184,166,0.08),transparent_70%)]"></div>
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-teal-400/40 to-transparent"></div>

      {/* 标题 */}
      <div className="relative flex items-center justify-between mb-2">
        <div className="flex items-center space-x-1.5">
          <div className="w-4 h-4 rounded bg-teal-500/20 border border-teal-400/50 flex items-center justify-center">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" className="text-teal-400">
        ...
        <div className="text-xs text-slate-400">PING检测</div>
      </div>

      {/* 设备列表 */}
      <div className="space-y-1">
        {wafData.map((device) => (
          <div key={device.name} className={`relative p-1.5 rounded border transition-all duration-300 ${
            device.status === 'online'
              ? 'bg-teal-950/40 border-teal-400/30 shadow-[0_0_6px_rgba(20,184,166,0.08)]'
              : 'bg-red-950/40 border-red-400/30 shadow-[0_0_6px_rgba(239,68,68,0.08)]'
          }`}>
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs px-1 py-0.5 rounded text-center min-w-[32px] ${
            ...
            {/* PING进度条 */}
            {device.status === 'online' && (
              <div className="mt-1 h-0.5 bg-slate-800/50 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-teal-500 to-teal-300 transition-all duration-1000 ease-linear"
                  style={{ width: `${pingProgress[device.name] || 0}%` }}
                ></div>
              </div>
            )}
          </div>
...
const IpsStatusComponent: React.FC = () => {
  ...
  return (
    <div className="relative bg-gradient-to-br from-violet-900/30 via-slate-900/80 to-purple-900/30 backdrop-blur-sm p-2.5 rounded-lg border border-violet-400/40 shadow-[0_0_15px_rgba(139,92,246,0.15)] overflow-hidden">
      {/* 背景装饰 */}
      ...
      <div className="relative flex items-center justify-between mb-2.5">
        <div className="flex items-center space-x-2">
          <div className="w-5 h-5 rounded bg-violet-500/20 border border-violet-400/50 flex items-center justify-center">
      ...
      <div className="space-y-1.5">
        {ipsData.map((device) => (
          <div key={device.name} className={`relative p-2 rounded border transition-all duration-300 ${
            device.status === 'online'
              ? 'bg-violet-950/40 border-violet-400/30 shadow-[0_0_8px_rgba(139,92,246,0.1)]'
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded text-center min-w-[36px] ${
            ...
            {device.status === 'online' && (
              <div className="flex items-center space-x-2">
                <span className="text-xs text-slate-400">下次PING:</span>
                <div className="flex-1 h-1 bg-slate-800/50 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-violet-500 to-violet-300 transition-all duration-1000 ease-linear"
                    style={{ width: `${pingProgress[device.name] || 0}%` }}
                  ></div>
                </div>
                <span className="text-xs text-violet-400 font-mono min-w-[20px]">
                  {Math.ceil(30 - (pingProgress[device.name] || 0) * 30 / 100)}s
                </span>
              </div>
            )}
          </div>
...
const DdosStatusComponent: React.FC = () => {
  ...
  return (
    <div className="relative bg-gradient-to-br from-amber-900/30 via-slate-900/80 to-orange-900/30 backdrop-blur-sm p-2.5 rounded-lg border border-amber-400/40 shadow-[0_0_15px_rgba(245,158,11,0.15)] overflow-hidden">
      {/* 背景装饰 */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_80%,rgba(245,158,11,0.1),transparent_70%)]"></div>
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-amber-400/50 to-transparent"></div>

      {/* 标题 */}
      <div className="relative flex items-center justify-between mb-2.5">
        <div className="flex items-center space-x-2">
          <div className="w-5 h-5 rounded bg-amber-500/20 border border-amber-400/50 flex items-center justify-center">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {ddosData.map((device) => {
          ...
          return (
            <div key={device.name} className={`relative p-1.5 rounded border transition-all duration-300 ${
              device.status === 'online'
                ? 'bg-amber-950/40 border-amber-400/30 shadow-[0_0_6px_rgba(245,158,11,0.1)]'
                : 'bg-red-950/40 border-red-400/30 shadow-[0_0_6px_rgba(239,68,68,0.1)]'
            }`}>
              {/* 设备名称与负载 */}
              <div className="flex items-center justify-between mb-1.5">
                ...
                {/* 负载指示器 */}
                <div className="relative w-1 h-3 bg-slate-800/80 rounded-sm overflow-hidden">
                  <div
      ...
      {/* 状态图例 */}
      <div className="flex justify-center items-center mt-2.5 space-x-3 text-xs border-t border-amber-400/20 pt-2">
        <div className="flex items-center">
          <div className="w-1 h-1 rounded mr-1 bg-green-400"></div>
          <span className="text-slate-400">正常</span>
        </div>
        <div className="flex items-center">
          <div className="w-1 h-1 rounded mr-1 bg-yellow-400"></div>
          <span className="text-slate-400">警告</span>
        </div>
        <div className="flex items-center">
          <div className="w-1 h-1 rounded mr-1 bg-red-400"></div>
