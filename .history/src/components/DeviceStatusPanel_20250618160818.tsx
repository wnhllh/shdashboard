...
const IpsStatusComponent: React.FC = () => {
  ...
  return (
    <div className="relative bg-gradient-to-br from-violet-900/30 via-slate-900/80 to-purple-900/30 backdrop-blur-sm p-2 rounded-lg border border-violet-400/40 shadow-[0_0_12px_rgba(139,92,246,0.12)] overflow-hidden">
      {/* 背景装饰 */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(139,92,246,0.08),transparent_70%)]"></div>
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-violet-400/40 to-transparent"></div>

      {/* 标题 */}
      <div className="relative flex items-center justify-between mb-2">
        <div className="flex items-center space-x-1.5">
          <div className="w-4 h-4 rounded bg-violet-500/20 border border-violet-400/50 flex items-center justify-center">
      ...
      <div className="space-y-1">
        {ipsData.map((device) => (
          <div key={device.name} className={`relative p-1.5 rounded border transition-all duration-300 ${
            device.status === 'online'
              ? 'bg-violet-950/40 border-violet-400/30 shadow-[0_0_6px_rgba(139,92,246,0.08)]'
              : 'bg-red-950/40 border-red-400/30 shadow-[0_0_6px_rgba(239,68,68,0.08)]'
          }`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1.5">
