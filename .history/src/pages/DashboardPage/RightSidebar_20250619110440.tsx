        ...
        {dashboardData.provinceWarnings && dashboardData.provinceWarnings.length > 0 ? (
          <div className="h-full overflow-hidden relative">
            ...
            <div className="absolute inset-0 z-0">
              <div className="h-full pt-32 overflow-hidden bg-black/30 rounded border border-green-500/20">
                <div className="flex items-center justify-between px-2 py-1 bg-green-900/30 border-b border-green-500/30">
                  <h3 className="text-xs text-green-400 font-medium">已闭环预警</h3>
                ...
                <div className="h-full overflow-y-auto px-1 pt-1">
                  {dashboardData.provinceWarnings.filter(warning => warning.is_closed).length > 0 ? (
                    dashboardData.provinceWarnings.filter(warning => warning.is_closed).slice(0, 6).map((warning) => (
                      <div key={`closed-${warning.id}`} className="flex items-center justify-between py-1 px-2 bg-green-900/30 rounded mb-1 border border-green-500/30 opacity-80" style={{height: '52px', minHeight: '52px'}}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1 mb-0.5">
                          ...
                          <h4 className="font-medium text-green-200 text-xs leading-tight truncate mb-0.5">{warning.title}</h4>
                          <div className="flex items-center gap-2 text-xxs text-slate-500">
                            <span>闭环: {warning.closed_by}</span>
                            <span>时间: {warning.closed_time ? new Date(warning.closed_time).toLocaleDateString() : ''}</span>
                            <span className="text-green-400">用时: {Math.floor((warning.response_time || 0) / 60)}h</span>
                          </div>
                        </div>
                        <div className="flex flex-col items-center ml-2">
                          <div className="w-2.5 h-2.5 bg-green-500 rounded-full mb-0.5"></div>
                          <span className="text-xxs text-green-400 font-bold">已闭环</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-slate-500 text-xs px-2 pt-2">暂无已闭环预警</p>
                  )}
                </div>
              </div>
            </div>
