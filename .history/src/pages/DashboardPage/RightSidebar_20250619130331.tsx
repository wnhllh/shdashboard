        ...
        <div className="space-y-3">
          ...
          <div className="bg-gradient-to-br from-slate-900/60 to-slate-800/40 rounded-xl p-4 border border-[#ffb74d]/20 shadow-lg backdrop-blur-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-[#ffb74d] flex items-center gap-2">
                <div className="w-2 h-2 bg-[#ffb74d] rounded-full animate-pulse"></div>
                <span>工作任务</span>
              </h3>
              <div className="flex items-center gap-3 text-xs">
                <span className="px-2 py-1 bg-orange-500/20 text-orange-300 rounded-full font-medium">
                  未反馈: {dashboardData.workTasks?.filter(t => t.feedback_status === '未反馈').length || 0}
                </span>
                <span className="px-2 py-1 bg-green-500/20 text-green-300 rounded-full font-medium">
                  已反馈: {dashboardData.workTasks?.filter(t => t.feedback_status === '已反馈').length || 0}
                </span>
              </div>
            </div>
            ...
            {dashboardData.workTasks && dashboardData.workTasks.length > 0 ? (
              <div className="space-y-3">
                {/* 状态可视化栏 */}
                <div className="bg-gradient-to-r from-slate-800/60 to-slate-700/40 rounded-lg p-3 border border-slate-600/40">
                ...
                {/* 未反馈任务滑动区 */}
                {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').length > 0 && (
                  <div className="h-32 overflow-hidden bg-gradient-to-br from-orange-900/30 to-red-900/20 rounded-xl border border-orange-500/40 shadow-inner">
                    <div
                      className="infinite-scroll-list"
                      style={{
                        animation: `workTasksUnfinishedScroll ${dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').length * 5}s infinite ease-in-out`
                      }}
                    >
                      <style>{`
                        @keyframes workTasksUnfinishedScroll {
                          ...
                          100% { transform: translateY(-${(dashboardData.workTasks?.filter(t => t.feedback_status === '未反馈').length || 0) * 60}px); }
                        }
                      `}</style>
                      {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                        <div key={`task-unfinished-${task.id}`} className="bg-gradient-to-r from-orange-800/40 to-orange-700/30 rounded-lg p-3 mb-2 border border-orange-500/50 shadow-lg backdrop-blur-sm" style={{height: '56px', minHeight: '56px'}}>
                          <div className="flex items-center justify-between h-full">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xxs font-bold px-2 py-0.5 rounded-full bg-orange-500/40 text-orange-100 border border-orange-400/50">未反馈</span>
                                <span className="text-xxs text-orange-300 font-mono font-bold">{task.task_id}</span>
                                <span className="text-xxs text-slate-400 bg-slate-700/50 px-1.5 py-0.5 rounded">{task.type}</span>
                              </div>
                              <h4 className="font-semibold text-orange-100 text-xs leading-tight truncate mb-1">{task.title}</h4>
                              <div className="flex items-center gap-3 text-xxs text-slate-300">
                                <span>发布: {new Date(task.publish_time).toLocaleDateString()}</span>
                                <span className="text-orange-300 font-medium">截止: {new Date(task.deadline_time).toLocaleDateString()}</span>
                              </div>
                            </div>
                            <div className="flex flex-col items-center ml-3">
                              <div className="relative">
                                <div className="w-3 h-3 bg-orange-500 rounded-full animate-pulse"></div>
                                <div className="absolute inset-0 w-3 h-3 bg-orange-400 rounded-full animate-ping opacity-40"></div>
                              </div>
                              <span className="text-xxs text-orange-400 font-bold mt-1">待处理</span>
                            </div>
                          </div>
                        </div>
                      ))}
                      {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                        <div key={`task-unfinished-repeat-${task.id}`} className="bg-gradient-to-r from-orange-800/40 to-orange-700/30 rounded-lg p-3 mb-2 border border-orange-500/50 shadow-lg backdrop-blur-sm" style={{height: '56px', minHeight: '56px'}}>
                          <div className="flex items-center justify-between h-full">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xxs font-bold px-2 py-0.5 rounded-full bg-orange-500/40 text-orange-100 border border-orange-400/50">未反馈</span>
                ...
                {/* 已反馈任务滑动区 */}
                <div className="h-24 overflow-hidden bg-gradient-to-br from-green-900/30 to-emerald-900/20 rounded-xl border border-green-500/40 shadow-inner">
                  <div
                    className="infinite-scroll-list"
                    style={{
                    ...
                    <style>{`
                      @keyframes workTasksFinishedScroll {
                        ${dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((_, index) => {
                          ...
                          const stayEndPercent = startPercent + stayPercent;
                          const endPercent = (index + 1) * itemDuration;
                          const currentPosition = -index * 45;
                          const nextPosition = -(index + 1) * 45;
                          return `
                            ${startPercent.toFixed(2)}% { transform: translateY(${currentPosition}px); }
                            ${stayEndPercent.toFixed(2)}% { transform: translateY(${currentPosition}px); }
                            ${endPercent.toFixed(2)}% { transform: translateY(${nextPosition}px); }
                        ...
                        100% { transform: translateY(-${(dashboardData.workTasks?.filter(t => t.feedback_status === '已反馈').length || 0) * 45}px); }
                      }
                    `}</style>
                    {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                      <div key={`task-finished-${task.id}`} className="bg-gradient-to-r from-green-800/30 to-green-700/20 rounded-lg p-2.5 mb-1.5 border border-green-500/40 shadow-md backdrop-blur-sm opacity-90" style={{height: '42px', minHeight: '42px'}}>
                        <div className="flex items-center justify-between h-full">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              <span className="text-xxs font-bold px-1.5 py-0.5 rounded-full bg-green-500/40 text-green-100 border border-green-400/50">已反馈</span>
                              <span className="text-xxs text-green-300 font-mono font-bold">{task.task_id}</span>
                              <span className="text-xxs text-slate-500 bg-slate-700/30 px-1 py-0.5 rounded">{task.type}</span>
                            </div>
                            <div className="flex items-center justify-between">
                              <h4 className="font-medium text-green-200 text-xs leading-tight truncate flex-1">{task.title}</h4>
                              {task.feedback_person && (
                                <span className="text-xxs text-green-400 ml-2">反馈人: {task.feedback_person}</span>
                              )}
                            </div>
                          </div>
                          <div className="flex flex-col items-center ml-2">
                            <div className="w-2.5 h-2.5 bg-green-500 rounded-full relative">
                              <div className="absolute inset-0.5 w-1.5 h-1.5 bg-white rounded-full"></div>
                            </div>
                            <span className="text-xxs text-green-400 font-bold mt-0.5">完成</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                      <div key={`task-finished-repeat-${task.id}`} className="bg-gradient-to-r from-green-800/30 to-green-700/20 rounded-lg p-2.5 mb-1.5 border border-green-500/40 shadow-md backdrop-blur-sm opacity-90" style={{height: '42px', minHeight: '42px'}}>
                        <div className="flex items-center justify-between h-full">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              <span className="text-xxs font-bold px-1.5 py-0.5 rounded-full bg-green-500/40 text-green-100 border border-green-400/50">已反馈</span>
          ...
          <div className="bg-gradient-to-br from-slate-900/60 to-blue-900/40 rounded-xl p-4 border border-blue-400/20 shadow-lg backdrop-blur-sm">
            ...
            {dashboardData.workNotifications && dashboardData.workNotifications.length > 0 ? (
              <div className="h-28 overflow-hidden bg-gradient-to-br from-blue-900/30 to-indigo-900/20 rounded-xl border border-blue-500/40 shadow-inner">
                <div
                  className="infinite-scroll-list"
                  style={{
                  ...
                  <style>{`
                    @keyframes workNotificationsScroll {
                      ${dashboardData.workNotifications.map((_, index) => {
                        ...
                        const stayEndPercent = startPercent + stayPercent;
                        const endPercent = (index + 1) * itemDuration;
                        const currentPosition = -index * 52;
                        const nextPosition = -(index + 1) * 52;
                        return `
                          ${startPercent.toFixed(2)}% { transform: translateY(${currentPosition}px); }
                          ${stayEndPercent.toFixed(2)}% { transform: translateY(${currentPosition}px); }
                          ${endPercent.toFixed(2)}% { transform: translateY(${nextPosition}px); }
                      ...
                      100% { transform: translateY(-${(dashboardData.workNotifications?.length || 0) * 52}px); }
                    }
                  `}</style>
                  {dashboardData.workNotifications.map((notification) => (
                    <div key={`notification-${notification.id}`} className="bg-gradient-to-r from-blue-800/40 to-blue-700/30 rounded-lg p-3 mb-2 border border-blue-500/50 shadow-lg backdrop-blur-sm" style={{height: '48px', minHeight: '48px'}}>
                      <div className="flex items-center justify-between h-full">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs text-blue-300 font-mono font-bold">{notification.notification_id}</span>
                            <span className="text-xxs text-blue-400 bg-blue-700/50 px-1.5 py-0.5 rounded font-medium">{notification.type}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <h4 className="font-semibold text-blue-100 text-xs leading-tight truncate flex-1">{notification.title}</h4>
                            <span className="text-xxs text-slate-400 ml-2">发布: {new Date(notification.publish_time).toLocaleDateString()}</span>
                          </div>
                        </div>
                        <div className="flex flex-col items-center ml-3">
                          <div className="w-2.5 h-2.5 bg-blue-500 rounded-full"></div>
                          <span className="text-xxs text-blue-400 font-bold mt-0.5">通知</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  {dashboardData.workNotifications.map((notification) => (
                    <div key={`notification-repeat-${notification.id}`} className="bg-gradient-to-r from-blue-800/40 to-blue-700/30 rounded-lg p-3 mb-2 border border-blue-500/50 shadow-lg backdrop-blur-sm" style={{height: '48px', minHeight: '48px'}}>
                      <div className="flex items-center justify-between h-full">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs text-blue-300 font-mono font-bold">{notification.notification_id}</span>
          ...
          <div className="bg-gradient-to-br from-slate-900/60 to-red-900/40 rounded-xl p-4 border border-red-400/20 shadow-lg backdrop-blur-sm">
            ...
            {dashboardData.attackSourceWarnings && dashboardData.attackSourceWarnings.length > 0 ? (
              <div className="h-28 overflow-hidden bg-gradient-to-br from-red-900/30 to-pink-900/20 rounded-xl border border-red-500/40 shadow-inner">
                <div
                  className="infinite-scroll-list"
                  style={{
                    animation: `attackSourceScroll ${dashboardData.attackSourceWarnings.length * 5}s infinite ease-in-out`
                  }}
                >
                  <style>{`
                    @keyframes attackSourceScroll {
                      ...
                      100% { transform: translateY(-${(dashboardData.attackSourceWarnings?.length || 0) * 52}px); }
                    }
                  `}</style>
                  {dashboardData.attackSourceWarnings.map((warning) => (
                    <div key={`attack-warning-${warning.id}`} className="bg-gradient-to-r from-red-800/40 to-red-700/30 rounded-lg p-3 mb-2 border border-red-500/50 shadow-lg backdrop-blur-sm" style={{height: '48px', minHeight: '48px'}}>
                      <div className="flex items-center justify-between h-full">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs text-red-300 font-mono font-bold">{warning.warning_id}</span>
                            <span className="text-xxs text-red-400 bg-red-700/50 px-1.5 py-0.5 rounded font-medium">点击: {warning.hits}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <h4 className="font-semibold text-red-100 text-xs leading-tight truncate flex-1">{warning.title}</h4>
                            <span className="text-xxs text-slate-400 ml-2">发布: {new Date(warning.publish_time).toLocaleDateString()}</span>
                          </div>
                        </div>
                        <div className="flex flex-col items-center ml-3">
                          <div className="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse"></div>
                          <span className="text-xxs text-red-400 font-bold mt-0.5">预警</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  {dashboardData.attackSourceWarnings.map((warning) => (
                    <div key={`attack-warning-repeat-${warning.id}`} className="bg-gradient-to-r from-red-800/40 to-red-700/30 rounded-lg p-3 mb-2 border border-red-500/50 shadow-lg backdrop-blur-sm" style={{height: '48px', minHeight: '48px'}}>
                      <div className="flex items-center justify-between h-full">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs text-red-300 font-mono font-bold">{warning.warning_id}</span>
