                  ...
                  >
                    <style>{`
                      @keyframes workTasksFinishedScroll {
                        ...
                        100% { transform: translateY(-${(dashboardData.workTasks?.filter(t => t.feedback_status === '已反馈').length || 0) * 60}px); }
                      }
                    `}</style>
                    {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                      <div key={`task-finished-${task.id}`} className="flex items-center justify-between py-1 px-3 bg-green-900/30 rounded mb-1 border border-green-500/30 opacity-80" style={{height: '55px', minHeight: '55px'}}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs font-bold px-1.5 py-0.5 rounded bg-green-500/30 text-green-200">已反馈</span>
                            <span className="text-xxs text-slate-500 font-medium">{task.task_id}</span>
                          </div>
                          <h4 className="font-medium text-green-200 text-xs leading-tight truncate">{task.title}</h4>
                        </div>
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                      </div>
                    ))}
                    {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                      <div key={`task-finished-repeat-${task.id}`} className="flex items-center justify-between py-1 px-3 bg-green-900/30 rounded mb-1 border border-green-500/30 opacity-80" style={{height: '55px', minHeight: '55px'}}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xxs font-bold px-1.5 py-0.5 rounded bg-green-500/30 text-green-200">已反馈</span>
                            <span className="text-xxs text-slate-500 font-medium">{task.task_id}</span>
