        ...
        <div className="space-y-4">
              ...
              <div className="p-3 space-y-3">
                ...
                {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').length > 0 && (
                  <div>
                    ...
                    <div className="h-48 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                      <div
                      ...
                      >
                        <style>{`
                          @keyframes workTasksUnfinishedScroll {
                            ${dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((_, index) => {
                              ...
                              const currentPosition = -index * 93;
                              const nextPosition = -(index + 1) * 93;
                              return `
                        ...
                        {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                          <div key={`task-unfinished-${task.id}`} className="rounded-lg p-3 mb-3 border border-orange-500/40" style={{height: '90px', minHeight: '90px'}}>
                            <div className="flex h-full">
                              {/* 左侧标签框 */}
                              <div className="flex-shrink-0 mr-3">
                                <div className="bg-orange-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                                  {task.type}
                                </div>
                              </div>
                              ...
                              <div className="flex-1 flex flex-col justify-between overflow-hidden">
                                {/* 标题 */}
                                <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-2">{task.title}</h4>

                                {/* 正文 */}
                                <p className="text-white text-xs leading-normal mb-2 flex-1 overflow-hidden">{task.description}</p>

                                {/* 底部信息栏 */}
                                <div className="flex items-center justify-between pt-1 border-t border-slate-600/30">
                                  <div className="flex items-center gap-3 text-xs">
                                    <span className="text-slate-300">发布: {new Date(task.publish_time).toLocaleDateString()}</span>
                                    <span className="text-orange-300">截止: {new Date(task.deadline_time).toLocaleDateString()}</span>
                                  </div>
                                  <span className="text-xs font-mono text-orange-400 bg-orange-500/10 px-2 py-1 rounded">{task.task_id}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                          <div key={`task-unfinished-repeat-${task.id}`} className="rounded-lg p-3 mb-3 border border-orange-500/40" style={{height: '90px', minHeight: '90px'}}>
                            <div className="flex h-full">
                              {/* 左侧标签框 */}
                              <div className="flex-shrink-0 mr-3">
                                <div className="bg-orange-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                ...
                <div>
                  ...
                  <div className="h-40 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                    <div
                      className="infinite-scroll-list p-2"
                      style={{
                    ...
                    >
                      ...
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-${task.id}`} className="rounded-lg p-3 mb-3 border border-green-500/40 opacity-80" style={{height: '90px', minHeight: '90px'}}>
                          <div className="flex h-full">
                            {/* 左侧标签框 */}
                            <div className="flex-shrink-0 mr-3">
                              <div className="bg-green-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                                {task.type}
                              </div>
                            </div>
                            ...
                            <div className="flex-1 flex flex-col justify-between overflow-hidden">
                              ...
                              <div className="flex items-center justify-between pt-1 border-t border-slate-600/30">
                                <div className="flex items-center gap-3 text-xs">
                                  <span className="text-slate-300">反馈人: {task.feedback_person}</span>
                                  <span className="text-slate-300">发布: {new Date(task.publish_time).toLocaleDateString()}</span>
                                </div>
                                <span className="text-xs font-mono text-green-400 bg-green-500/10 px-2 py-1 rounded">{task.task_id}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-repeat-${task.id}`} className="rounded-lg p-3 mb-3 border border-green-500/40 opacity-80" style={{height: '90px', minHeight: '90px'}}>
                          <div className="flex h-full">
                            {/* 左侧标签框 */}
                            <div className="flex-shrink-0 mr-3">
                              <div className="bg-green-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                ...
                <div className="h-48 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                  ...
                  >
                    ...
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-${notification.id}`} className="rounded-lg p-3 mb-3 border border-blue-500/40" style={{height: '90px', minHeight: '90px'}}>
                        <div className="flex h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0 mr-3">
                            <div className="bg-blue-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                              {notification.type}
                            </div>
                          </div>
                          ...
                          <div className="flex-1 flex flex-col justify-between overflow-hidden">
                            {/* 标题 */}
                            <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-2">{notification.title}</h4>

                            {/* 正文 */}
                            <p className="text-white text-xs leading-normal mb-2 flex-1 overflow-hidden">{notification.description}</p>

                            {/* 底部信息栏 */}
                            <div className="flex items-center justify-between pt-1 border-t border-slate-600/30">
                              <div className="flex items-center gap-3 text-xs">
                                <span className="text-slate-300">发布: {new Date(notification.publish_time).toLocaleDateString()}</span>
                              </div>
                              <span className="text-xs font-mono text-blue-400 bg-blue-500/10 px-2 py-1 rounded">{notification.notification_id}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-repeat-${notification.id}`} className="rounded-lg p-3 mb-3 border border-blue-500/40" style={{height: '90px', minHeight: '90px'}}>
                        <div className="flex h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0 mr-3">
                            <div className="bg-blue-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
          ...
          <div className="bg-slate-900/80 backdrop-blur-md rounded-lg border border-slate-700/50">
            <div className="px-3 py-2 flex items-center justify-between">
              ...
              <span className="px-2 py-1 bg-red-500/20 text-red-300 rounded border border-red-500/30 text-xs">
            ...
            {dashboardData.attackSourceWarnings && dashboardData.attackSourceWarnings.length > 0 ? (
              <div className="p-3">
                <div className="h-48 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                  <div
                    className="infinite-scroll-list p-2"
                    style={{
                      animation: `attackSourceScroll ${dashboardData.attackSourceWarnings.length * 6}s infinite ease-in-out`
                    }}
                  >
                    <style>{`
                    ...
                    {dashboardData.attackSourceWarnings.map((warning) => (
                      <div key={`attack-warning-${warning.id}`} className="rounded-lg p-3 mb-3 border border-red-500/40" style={{height: '90px', minHeight: '90px'}}>
                        <div className="flex h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0 mr-3">
                            <div className="bg-red-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
                              攻击源
                            </div>
                          </div>
                          ...
                          <div className="flex-1 flex flex-col justify-between overflow-hidden">
                            {/* 标题 */}
                            <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-2">{warning.title}</h4>

                            {/* 正文 */}
                            <p className="text-white text-xs leading-normal mb-2 flex-1 overflow-hidden">{warning.description}</p>

                            {/* 底部信息栏 */}
                            <div className="flex items-center justify-between pt-1 border-t border-slate-600/30">
                              <div className="flex items-center gap-3 text-xs">
                                <span className="text-slate-300">发布: {new Date(warning.publish_time).toLocaleDateString()}</span>
                                <span className="text-slate-300">点击: {warning.hits}</span>
                              </div>
                              <span className="text-xs font-mono text-red-400 bg-red-500/10 px-2 py-1 rounded">{warning.warning_id}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {dashboardData.attackSourceWarnings.map((warning) => (
                      <div key={`attack-warning-repeat-${warning.id}`} className="rounded-lg p-3 mb-3 border border-red-500/40" style={{height: '90px', minHeight: '90px'}}>
                        <div className="flex h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0 mr-3">
                            <div className="bg-red-500 text-white text-xs font-bold px-3 py-2 rounded-md shadow-sm">
              ...
              <div className="p-3">
                <p className="text-slate-500 text-xs text-center">暂无攻击源预警</p>
              </div>
            )}
          </div>
