      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/80 backdrop-blur-md rounded-lg border border-slate-700/50">
            ...
            {dashboardData.workTasks && dashboardData.workTasks.length > 0 ? (
              <div className="p-3 space-y-3">
                    ...
                    <div className="h-44 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                      ...
                      >
                        ...
                        {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                          <div key={`task-unfinished-${task.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-slate-600/50 transition-colors" style={{height: '80px', minHeight: '80px'}}>
                            <div className="flex items-start justify-between h-full">
                              <div className="flex-1 min-w-0 pr-2">
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className="text-xs px-2 py-0.5 bg-slate-600/50 text-slate-300 rounded border border-slate-500/50">{task.type}</span>
                                </div>
                                <h4 className="font-semibold text-orange-400 text-sm leading-tight mb-2">{task.title}</h4>
                              ...
                              <div className="flex flex-col items-center justify-center">
                                ...
                                <span className="text-xs text-orange-400 mt-1">待处理</span>
                              </div>
                            </div>
                          </div>
                        ))}
                        {dashboardData.workTasks.filter(t => t.feedback_status === '未反馈').map((task) => (
                          <div key={`task-unfinished-repeat-${task.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-slate-600/50 transition-colors" style={{height: '80px', minHeight: '80px'}}>
                            <div className="flex items-start justify-between h-full">
                              <div className="flex-1 min-w-0 pr-2">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-xs font-mono text-orange-300">
                ...
                {/* 已完成任务 */}
                <div>
                  <div className="text-xs text-slate-400 mb-2 px-1">已完成任务</div>
                  <div className="h-28 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                    <div
                      className="infinite-scroll-list p-2"
                      style={{
                    ...
                    >
                      <style>{`
                        @keyframes workTasksFinishedScroll {
                          ...
                          100% { transform: translateY(-${(dashboardData.workTasks?.filter(t => t.feedback_status === '已反馈').length || 0) * 55}px); }
                        }
                      `}</style>
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-${task.id}`} className="bg-slate-800/40 hover:bg-slate-700/40 rounded-lg p-2.5 mb-2 border border-slate-600/30 transition-colors opacity-80" style={{height: '50px', minHeight: '50px'}}>
                          <div className="flex items-start justify-between h-full">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-mono px-2 py-0.5 bg-green-500/20 text-green-300 rounded">
                                  {task.task_id}
                                </span>
                                <span className="text-xs px-1.5 py-0.5 bg-slate-600/50 text-slate-400 rounded">{task.type}</span>
                              </div>
                              <div className="flex items-center justify-between">
                                <h4 className="font-medium text-slate-300 text-xs truncate flex-1">{task.title}</h4>
                                <span className="text-xs text-green-400 ml-2">✓ {task.feedback_person}</span>
                              </div>
                            </div>
                      ...
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-repeat-${task.id}`} className="bg-slate-800/40 hover:bg-slate-700/40 rounded-lg p-2.5 mb-2 border border-slate-600/30 transition-colors opacity-80" style={{height: '50px', minHeight: '50px'}}>
                          <div className="flex items-start justify-between h-full">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-mono px-2 py-0.5 bg-green-500/20 text-green-300 rounded">
          ...
          <div className="bg-slate-900/80 backdrop-blur-md rounded-lg border border-slate-700/50 overflow-hidden">
            <div className="px-4 py-3 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-b border-slate-700/50">
              <div className="flex items-center justify-between">
                  ...
                  >
                    <style>{`
                      @keyframes workNotificationsScroll {
                        ...
                        100% { transform: translateY(-${(dashboardData.workNotifications?.length || 0) * 70}px); }
                      }
                    `}</style>
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-${notification.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-slate-600/50 transition-colors" style={{height: '65px', minHeight: '65px'}}>
                        <div className="flex items-start justify-between mb-1">
                          <span className="text-xs font-mono px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded">
                            {notification.notification_id}
                          </span>
                          <span className="text-xs px-1.5 py-0.5 bg-slate-600/50 text-slate-300 rounded">{notification.type}</span>
                        </div>
                        <h4 className="font-medium text-slate-200 text-xs leading-tight mb-1 line-clamp-2">{notification.title}</h4>
                    ...
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-repeat-${notification.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-slate-600/50 transition-colors" style={{height: '65px', minHeight: '65px'}}>
                        <div className="flex items-start justify-between mb-1">
                          <span className="text-xs font-mono px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded">
                            {notification.notification_id}
                          </span>
