                  ...
                  <div className="h-36 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                    ...
                    >
                      ...
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-${task.id}`} className="bg-slate-800/40 hover:bg-slate-700/40 rounded-lg p-3 mb-2 border border-green-500/30 transition-colors opacity-80" style={{height: '85px', minHeight: '85px'}}>
                          <div className="flex gap-3 h-full">
                            {/* 左侧标签框 */}
                            <div className="flex-shrink-0">
                              <div className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
                                {task.type}
                              </div>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-1 truncate">{task.title}</h4>
                              <p className="text-white text-xs leading-relaxed mb-2 line-clamp-2">{task.description}</p>
                              <div className="flex items-center justify-between text-xs text-slate-400">
                                <span>反馈人: {task.feedback_person}</span>
                                <span>发布: {new Date(task.publish_time).toLocaleDateString()}</span>
                                <span className="text-green-300">{task.task_id}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {dashboardData.workTasks.filter(t => t.feedback_status === '已反馈').map((task) => (
                        <div key={`task-finished-repeat-${task.id}`} className="bg-slate-800/40 hover:bg-slate-700/40 rounded-lg p-3 mb-2 border border-green-500/30 transition-colors opacity-80" style={{height: '85px', minHeight: '85px'}}>
                          <div className="flex gap-3 h-full">
                            {/* 左侧标签框 */}
                            <div className="flex-shrink-0">
                              <div className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
                ...
                <div className="h-44 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                  ...
                  >
                    ...
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-${notification.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-blue-500/30 transition-colors" style={{height: '85px', minHeight: '85px'}}>
                        <div className="flex gap-3 h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0">
                            <div className="bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
                              {notification.type}
                            </div>
                          </div>
                          ...
                          {/* 右侧内容 */}
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-1 truncate">{notification.title}</h4>
                            <p className="text-white text-xs leading-relaxed mb-2 line-clamp-2">{notification.description}</p>
                            <div className="flex items-center justify-between text-xs text-slate-400">
                              <span>发布: {new Date(notification.publish_time).toLocaleDateString()}</span>
                              <span className="text-blue-300">{notification.notification_id}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {dashboardData.workNotifications.map((notification) => (
                      <div key={`notification-repeat-${notification.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-blue-500/30 transition-colors" style={{height: '85px', minHeight: '85px'}}>
                        <div className="flex gap-3 h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0">
                            <div className="bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
                ...
                <div className="h-44 overflow-hidden bg-slate-800/30 rounded-lg border border-slate-700/50">
                  ...
                  >
                    ...
                    {dashboardData.attackSourceWarnings.map((warning) => (
                      <div key={`attack-warning-${warning.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-red-500/30 transition-colors" style={{height: '85px', minHeight: '85px'}}>
                        <div className="flex gap-3 h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0">
                            <div className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
                              攻击源
                            </div>
                          </div>
                          ...
                          {/* 右侧内容 */}
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-yellow-400 text-sm leading-tight mb-1 truncate">{warning.title}</h4>
                            <p className="text-white text-xs leading-relaxed mb-2 line-clamp-2">{warning.description}</p>
                            <div className="flex items-center justify-between text-xs text-slate-400">
                              <span>发布: {new Date(warning.publish_time).toLocaleDateString()}</span>
                              <span>点击: {warning.hits}</span>
                              <span className="text-red-300">{warning.warning_id}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {dashboardData.attackSourceWarnings.map((warning) => (
                      <div key={`attack-warning-repeat-${warning.id}`} className="bg-slate-800/60 hover:bg-slate-700/60 rounded-lg p-3 mb-2 border border-red-500/30 transition-colors" style={{height: '85px', minHeight: '85px'}}>
                        <div className="flex gap-3 h-full">
                          {/* 左侧标签框 */}
                          <div className="flex-shrink-0">
                            <div className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">
